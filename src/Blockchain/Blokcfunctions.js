...
import {loadWeb3} from './LoadSmartContract'
...
const stopDeposit = async (USDC_address, assetToDeposit, value, dip, userAddress) => {
  ...
  let events =   await contractMain.methods.stopLoss_deposit(
        USDC_address, 
        assetToDeposit, 
        valueWei, 
        convertDip).send({from: userAddress , value:0}).then(console.log);
    console.log('SC function has been called'+events);
};
...
const approve_WETH = async ( _value, userAddress) => {
    ...
    const _wei = web3.utils.toWei(_value,'ether');
...
const approve_USDC = async ( _value, userAddress) => {
    ...
    const ERC20 = contractERC20.methods;
    await ERC20.approve(
        '0x20c5626017955109844eA597391e07A6672DE8A6',
        _wei).send({from: userAddress,value:0});
    console.log('approve()  function has been called');
